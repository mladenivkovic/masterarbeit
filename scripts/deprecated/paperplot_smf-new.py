...
fig_reduced = None
axes_reduced = None
#  zmean_keep = [0.10, 0.25, 0.35, 0.45, 0.57, 0.70, 0.90, 1.15, 1.25]
rows_red = 3
cols_red = 3

ax_for_reduced = [ True, True, False, True,
...
#===============================
    ...
    for i in range(uniques):
        ...
        ax = axes[i]
        plt.sca(ax)
        ax.plot(masses_plot[mask], sm, c=clist[lineindex], ls=ls[lineindex],
            label=label)
        ax.legend(prop=fontP)


        if ax_for_reduced[i]:
            ax = axes_reduced[ax_ind_in_reduced[i]]
            plt.sca(ax)
            ax.plot(masses_plot[mask], sm, c=clist[lineindex], ls=ls[lineindex],
                label=label)
            ax.legend(prop=fontP)
...
#========================
    ...
    global fig_reduced, axes_reduced
    ...
    fig = plt.figure(1, figsize=(12,15),dpi=300)
    ...
    fig_reduced = plt.figure(2, figsize=(12, 12), dpi=300)
    axes_reduced = [None for i in range(rows_red*cols_red)]
    for i in range(rows_red*cols_red):
        axes_reduced[i] = fig_reduced.add_subplot(rows_red, cols_red, i+1)
...
#========================
    ...
    plt.savefig('paper-smf_both_sims.pdf', format='pdf')
    ...
    for r in range(rows_smf):
        for c in range(cols_smf):
            ...
            if ind<uniques:
                if ax_for_reduced[ind]:

                    ax = axes_reduced[ax_ind_in_reduced[ind]]
                    plt.sca(ax)
    ...
    plt.savefig('paper-smf_both_sims-reduced.pdf', format='pdf')
