...
#
# Same as evalplot.py, but differentiates between
# haloes and subhaloes.
# Haloes are solid lines, subhaloes are dashed lines.
#===================================================
...
#===============================================
    ...
    while l < len(tikzlines):
        if tikzlines[l].startswith("\\path"):
            ...
            if special_case == 'mgf':
                newline = tikzlines[l][:-2]+', horizontal sep=2cm, vertical sep = 3cm}]\n'
            else:
...
#===================
    ...
    #  allfiles = ['eval_trees-ntrace-'+str(i)+'-subhaloes.txt' for i in ntrace]
    #  labelnames = [r'$n_{mb}='+str(i)+'$' for i in ntrace]

    allfiles = [ 'eval_trees_subhalo-inc-nosaddle.txt',  'eval_trees_subhalo-exc-nosaddle.txt', 'eval_trees_subhalo-inc-saddle.txt', 'eval_trees_subhalo-exc-saddle.txt' ]
    ...
    for f,srcfname in enumerate(allfiles):
        ...
        for i in range(1,bins.shape[0]):
            ...
            axgroup1[i-1].semilogy(mblens[1:], mblen_counts[1:], lw=2, label=labelnames[f], alpha=0.6, zorder=f+1, c=colors[f])
        ...
        for i in range(1,bins.shape[0]):
            ...
            axgroup2[i-1].loglog(nbranches, nbranch_counts, label=labelnames[f], lw=2, alpha=0.7, c=colors[f])
        ...
        ax9.semilogy(mass_growth, mass_growth_counts, label=labelnames[f], lw=2, ls='--', alpha=0.7,c=colors[f])
        ...
        ax11.semilogy(mass_growth, mass_growth_counts, lw=2, ls='--', alpha=0.7,c=colors[f], label=labelnames[f])
        ...
        ax10.semilogy(mass_flucts, mass_fluct_counts, c=colors[f], lw=2, ls='--', alpha=0.7, label=labelnames[f])
        ...
        ax12.semilogy(mass_flucts, mass_fluct_counts, c=colors[f], lw=2, ls=':', alpha=0.7, label=labelnames[f])
        ...
        ax13.semilogy(displacements, displacement_counts, label=labelnames[f], lw=2, ls='-', c=colors[f], alpha=0.72)
        ...
        ax13.semilogy(displacements, displacement_counts, lw=2, ls='--', c=colors[f], alpha=0.7)
    ...
    #  ax = ax1.twiny()
    #  ax.set_xlim(0.5, maxlen+0.5)
    #  ax.set_xticks([i for i in range(xmax-1, xstep)])
    #  ax.set_xticklabels(["%.2f" % abs(redshift[i]) for i in range(xmax-1, xstep)])
    ...
    ax9.set_title("Logarithmic Mass Growth of Haloes")
    ...
    ax10.set_title("Mass Growth Fluctuations of Haloes")
    ...
    ax11.set_title("Logarithmic Mass Growth of Subhaloes")
    ax11.set_xlim(mgmin*1.1, mgmax*1.1)
    ax11.set_xticks(np.linspace(-1,1, 11))
    ax11.set_xticklabels(["%.2f" % i for i in np.linspace(-1,1, 11)])
    ax11.set_xlabel(r'$\beta_M = \frac{2}{\pi}\arctan\frac{d \log M}{d \log t}$')
    ylabelmin = int(np.log10(mgcountmin))
    ylabelmax = int(np.log10(mgcountmax)+0.5)
    ax11.set_ylim(mgcountmin/2, mgcountmax*2)
    ...
    ax11.legend(loc='lower right', ncol=ncols)
    ax11.grid()

    ax12.set_title("Mass Growth Fluctuations of Subhaloes")
    ax12.set_xlim(mfmin*1.1, mfmax*1.1)
    xtickstart = int(mfmin*10-0.5)/10
    xtickend = int(mfmax*10+0.5)/10
    xticks = np.linspace(xtickstart, xtickend, int((xtickend-xtickstart)/0.2+0.5)+1)
    ax12.set_xticks(xticks)
    ax12.set_xticklabels(["%.2f" % x for x in xticks])
    ax12.set_xlabel(r'$\xi_M = \frac{\beta_M(k, k+1) - \beta_M(k-1, k)}{2}$')
    ylabelmin = int(np.log10(mfcountmin))
    ylabelmax = int(np.log10(mfcountmax)+0.5)
    ax12.set_ylim(mfcountmin/2, mfcountmax*2)
    ax12.set_yticks([10.0**i for i in range(ylabelmin, ylabelmax, 1)])
    ax12.set_yticklabels([r"$10^{"+str(i)+"}$" for i in range(ylabelmin, ylabelmax, 1)])
    ax12.set_ylabel(r'$N/N_{tot}$')
    ax12.legend(loc='upper right', ncol=ncols)
    ax12.grid()
    ...
    figh = "18cm"
    figw = "18cm"
    save_figures(figname, fig3, figh, figw, special_case='mgf')
    ...
    ax13.legend(ncol=ncols)
    ax13.grid()
    ax13.set_ylabel(r'$N/N_{tot}$')
    ax13.set_xlabel(r'$\Delta r$')
    ax13.set_xlim(0.005, displmax+displmax/10)
    ax13.set_title("Displacements")

    xtickstart = 0
    xtickend = int(displmax*10.1)/10
    xticks = np.linspace(xtickstart, xtickend, int(xtickend/0.3+0.5)+1)
    ax13.set_xticks(xticks)
    ax13.set_xticklabels(["%.1f" % x for x in xticks])

    ylabelmin = int(np.log10(dcountmin))
    ylabelmax = int(np.log10(dcountmax)+0.5)
    ax13.set_ylim(dcountmin/2, dcountmax*2)
    ax13.set_yticks([10.0**i for i in range(ylabelmin, ylabelmax, 1)])
    ax13.set_yticklabels([r"$10^{"+str(i)+"}$" for i in range(ylabelmin, ylabelmax, 1)])
