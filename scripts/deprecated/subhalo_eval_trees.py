...
# them to file so they can be plotted en masse with subhalo_eval_plots.py
#
# Same as eval_trees.py, but differentiates between
...
mass_growth = 0
mass_growth_subhalos = 0
mass_flucts = 0
mass_flucts_subhalos = 0
mffree = 0
mfsubhalosfree = 0
displacements = 0
displacements_subhalos = 0
displfree = 0
displsubhalofree = 0
mthresh = 0
...
#===============================================
    ...
    global mass_growth_subhalos, mass_flucts_subhalos, mfsubhalosfree
    global displacements, displacements_subhalos
    global displfree, displsubhalofree
    ...
    if mass[o1][i1] > mthresh and mass[o2][i2] > mthresh:
        ...
        if descendants[o1][i1] in halos[o1]:
            ...
            if mass_growth[o3][i3] < 2.0 and i3 >= 0: #don't allow default i3=-1
                ...
                mffree += 1
            elif mass_growth_subhalos[o3][i3] < 2.0 and i3 >= 0: #don't allow default i3=-1
                mass_flucts[mffree] = (mass_growth_subhalos[o3][i3] - mass_growth[o1][i1])*0.5
                mffree += 1
            ...
            displacements_subhalos[displsubhalofree] = disp
            displsubhalofree+= 1

            mass_growth_subhalos[o1][i1] = 2.0/np.pi*np.arctan((mass[o1][i1]-mass[o2][i2])*(times[o1]+times[o2])/((mass[o1][i1]+mass[o2][i2])*(times[o1]-times[o2])))


            # calc mass fluctuation
            if mass_growth_subhalos[o3][i3] < 2.0 and i3 >= 0: #don't allow default i3=-1
                mass_flucts_subhalos[mfsubhalosfree] = (mass_growth_subhalos[o3][i3] - mass_growth_subhalos[o1][i1])*0.5
                mfsubhalosfree+= 1
...
#====================================
    ...
    outfname = 'eval_trees_subhaloes.txt'
    ...
    mgs = np.concatenate([mgs_out[mgs_out<2.0] for mgs_out in mass_growth_subhalos])
    mgsplot, binedges = np.histogram(mgs, bins=bins2s)
    mgsplot = mgsplot*1.0
    mgsplot /= mgsplot.sum() # normalize
    write_array(outfile, mgsplot, (binedges[1:]+binedges[:-1])/2.0)
    ...
    mfs = mass_flucts_subhalos[mass_flucts_subhalos < 2.0]
    mfsplot, binedges = np.histogram(mfs, bins=bins2s)
    mfsplot = mfsplot*1.0
    mfsplot /= mfsplot.sum() # normalize
    write_array(outfile, mfsplot, (binedges[1:]+binedges[:-1])/2.0)
    ...
    dispss = displacements_subhalos[displacements_subhalos != 0]
    dispssplot, binedges = np.histogram(dispss, bins=bins2)
    dispssplot = dispssplot * 1.0
    dispssplot /= dispssplot.sum() # normalize
    write_array(outfile, dispssplot, (binedges[1:]+binedges[:-1])/2.0)
...
#============================================
    ...
    halos = [np.zeros(1) for i in range(noutput)]
    ...
    for output in range(noutput):
        ...
        if output < noutput -1: # don't try to read progenitor stuff from output_00001
            ...
            try:
                ...
                halolist = [np.zeros(1) for i in range(ncpu)]
                i = 0
                for f in fnames:
                ...
                # Read in halo data
                #------------------------------
                fnames = [srcdir + '/' + 'halo_'+dirnr+'.txt' + str(cpu+1).zfill(5) for cpu in range(ncpu)]
                j = 0
                for f in fnames:
                    try :
                        halolist[j] = np.atleast_1d(np.loadtxt(f, skiprows=1, usecols=([1]), dtype='int'))
                        j += 1
                    except Warning:
                        continue
                ...
                if j > 0:
                   fullhalo = np.concatenate(halolist[:j], axis=0)
                   halos[output] = fullhalo
...
#====================
    ...
    mass_flucts_subhalos = np.ones(nhalosmax*noutput)*2.0
    mffree = 0 # index for mass flucts
    mfsubhalosfree = 0
    mass_growth = [np.ones(nhalosmax, dtype='float')*2.0 for i in range(noutput)]
    mass_growth_subhalos = [np.ones(nhalosmax, dtype='float')*2.0 for i in range(noutput)]

    displacements = np.zeros(nhalosmax*noutput)
    displacements_subhalos = np.zeros(nhalosmax*noutput)
    displfree = 0
    displsubhalofree = 0
