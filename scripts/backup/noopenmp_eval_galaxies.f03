program eval_galaxies
  ...
  ! do c = 1, 2
    ...
    do i=1, nc/2+1
      do j=1, nc
        do k=1, nc
          Pk_field(i,j,k) = (real(Pk_field(i,j,k))**2 + aimag(Pk_field(i,j,k))**2)/nccube**2
        enddo
      enddo
    enddo
      ...
      do j = 1, nc
        do k = 1, nc
          ...
          if (d<=kmax .and. d>0.d0) then
            ...
            enddo
            if (i==1 .or. i==nc/2+1) then; f=1; else; f=2; endif
    ...
    do i=1, nc
      do j=1, nc
        do k=1, nc
          density_field(i,j,k) = density_field(i,j,k)
