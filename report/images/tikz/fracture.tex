...
\begin{document}
	\begin{tikzpicture}[xscale=1,yscale=1,samples=400, transform shape,every node/.style={scale=1.2}, shorten <=1pt, shorten >=1pt]
        ...
        \draw node (A2) at (13,0) [fill=gray!60,circle,minimum size=0.5cm, draw]{$B_2$};
        \draw node (P) [inner sep=0,minimum size=0.01,fill=black,circle,draw] at (12,2) {};        
        
        % Left Side
        \draw node (B1) at (8,4) [fill=gray!60,circle,minimum size=1.3cm, draw]{$A_2$}; 
        \draw[-stealth', line width=2pt] (B1) to (A1);      
       
        % Right Side    
        \draw node (B2) at (12,4) [fill=gray!60,circle,minimum size=1.2cm, draw]{$A_1$};
